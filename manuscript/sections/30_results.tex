...
CD4\textsuperscript{+} T cells yields $33{,}983$ perturbation--condition effect profiles over
$10{,}282$ genes, filtered to $6{,}871$ generators for the resting analysis. \textbf{(B)}~Reachability is membership in the non-negative
conic hull of these vectors; the reachable component is the NNLS projection of the target,
and the residual is the outside-the-cone direction, with ``outside'' certified by a
...
dual certificate ranking positive unmet readouts. \textbf{(D)}~The identical operator,
...
far above the shuffled-target null at every recipe size. The fixed-split held-out-gene cosine
is $0.448$ against a null of mean $-0.005$ and standard deviation $0.019$; it exceeds all $60$
shuffled targets (empirical $p=1/61$; descriptive $z\approx24$). Thus the fitted mixture carries
target-specific signal onto expression coordinates it was not fit on, yet the absolute value
is well below $1$. The score is stable to the gene partition: across $12$ fixed half-gene
splits, held-out cosine has mean $0.4464$, standard deviation $0.0101$, and range
$0.4326$--$0.4587$. This is the central
...
shift lies in the knockdown cone; $25\%$ is captured by the sign-flipped GOF proxy, and the
remaining $35\%$ aligns with neither modelled cone. The dual certificate
(Fig.~\ref{fig:headline}c) ranks seven leading positive unmet readouts---LYAR, IKZF3, CRTAM,
CBLB, GBP5, LAG3, and IRF8. Together the residual certifies that the full target direction is
outside the measured knockdown cone. Individually, these genes are follow-up CRISPRa or
de-repression hypotheses, not proven causal activations.
...
cosine of $0.448$ (larger than all $60$ shuffled targets; empirical $p=1/61$).
\textbf{(b)}~Staged decomposition of the target: $39\%$ reachable by measured knockdown,
$25\%$ captured by sign-flipped GOF proxy atoms, $35\%$ neither. \textbf{(c)}~Dual
certificate: seven leading positive unmet readouts---falsifiable hypotheses in expression
space, with a realizing intervention differing by gene role (activation for effectors,
...
$12$ targets exceeds its eight shuffled screening controls, with descriptive held-out $z$
ranging from $4.7$ to $45.0$. Because eight permutations cannot estimate a conventional tail
probability, these atlas values support prioritization rather than formal per-cell significance.
In no case does loss-of-function account
for the majority of the shift (Fig.~\ref{fig:atlas}a): the mean loss-of-function fraction is
$0.34$, and the knockdown bar never crosses the $50\%$ line. Resting cells have the highest
held-out cosine along all four axes, showing strong context dependence, and the verdict grades
range from ``partially reachable'' (four resting
...
transition from its held-out $z$ ($2$ high, $6$ medium, $4$ low) and use held-out numbers for
headline comparisons. Finally, the closed-form anisotropy null lands almost exactly on the empirical
...
majority line, mean LOF fraction $0.34$. All targets exceed their eight shuffled controls;
the displayed $z$ range ($4.7$--$45.0$) is descriptive, not a formal permutation tail test.
...
and is the sole source of the Farkas/KKT certificate. The held-out cosine can also be compared
with the in-sample cone fit: the headline $0.448$ retains $71\%$ of the in-sample $0.627$
(atlas mean $61\%$, range $49$--$71\%$). This is a descriptive generalization ratio, not an
independent modality ceiling or a biological bound. Under the magnitude-saturation model
transferred from Norman CRISPRa doubles, every knee recipe receives a predicted reliability of
$0.92$--$0.96$ across the $12$ cells, and the combined-magnitude
saturation cap binds only near $k\approx28$ knockdowns against a knee of $k\approx4$--$5$, a
$5.6\times$ modelled margin on the headline recipe. These are risk annotations, not in-domain
validation of CD4\textsuperscript{+} multi-knockdown additivity.

Two sharper challenges aimed at the evaluation itself leave the conclusion stronger rather than
weaker. The signal-dilution critique of \citet{Mejia2026}---that a uniform-gene metric averages
...
paralog, with a knee at $k=2$ (CEBPE\,$+$\,SPI1)---a compact, biologically coherent candidate
panel rather than a global minimum-cardinality proof.
...
largest residual coordinates mark a myeloid program (MNDA, HP, VSIG4, NCF1); the full
residual is the certificate. \textbf{(b)}~Across $126$ measured
...
The greedy sparse \emph{recipe} does \emph{not} transfer: the same-target Jaccard overlap across cell
...
across cell types, but the \emph{specific candidate recipe} is cell-type-specific and must be
...
\textbf{(b)}~Across the atlas, the union of top-10 greedy LOF panels contains $102$ unique
candidate knockdown nodes ($45/102 = 44\%$ hard-to-drug, $10$ with a clinical-grade drug);
state-level verdicts determine GO/REDIRECT/STOP, while candidate-level annotations prioritize
what to test within a chosen arm. All nominations
...
A feasibility verdict earns its keep by changing what experiment gets run, but two decision
levels must remain separate. At the \emph{state} level, a strong knockdown component supports
\textbf{GO} to a focused CRISPRi combination test; a substantial sign-flipped GOF component
supports \textbf{REDIRECT} to a CRISPRa/de-repression arm; and a direction indistinguishable
from the null supports \textbf{STOP} pending a better dictionary or modality. A non-zero
geometric residual alone is not a clinical stop rule. At the \emph{candidate} level, the
union of top-10 greedy LOF panels across the 12 state--condition cases contains $102$ unique
knockdown nodes. We annotate these candidates, rather than assigning each a reachability
verdict: $45/102$ ($44\%$) lack a conventional handle and $10$ carry a clinical-grade drug.
Across transitions, the sign-flipped proxy accounts for $36$--$49\%$ (mean $\approx42\%$) of
the LOF+GOF-addressable share, motivating a second modality arm but not estimating how much
experimental effort would otherwise be wasted.

The candidate annotation surfaces concrete, and honestly caveated, examples. Nine LOF nodes
combine a tractable handle with immune-genetic evidence (including JAK2, ICOS, MAPK14, CD3D).
IL7R, ZAP70, and TET2 are tractable but lack a clinical-grade drug in this snapshot and are
priorities for experimental tool selection, not automatic CRISPRa redirects. IRF1 is a top
T\textsubscript{H}2 knockdown node with strong immune-disease
